...
import { LogOut, Loader, AlertTriangle, WifiOff } from 'lucide-react';
...
export function RepLayoutClient({ children }: { children: ReactNode }) {
    ...
    // Auto-redirect based on connection
    useEffect(() => {
        ...
        return (
            <div className="flex h-screen flex-col items-center justify-center bg-background p-4 text-center">
                ...
                <p className="text-muted-foreground mb-6">
                    It's taking longer than expected to load your profile. Please try logging in again.
                </p>
                <Button
                    onClick={() => {
                        auth?.signOut();
            ...
            <header className="sticky top-0 z-20 flex h-16 items-center justify-between border-b border-border bg-background/80 px-4 backdrop-blur-sm lg:px-6">
                <div className="flex items-center gap-4">
                    <Link href={isOfflineMode ? '/rep/offline' : '/rep'} className="flex items-center gap-2">
                        <img src="/icon-192.png" alt="SG HEALTH PHARMA Logo" className="h-8 w-8 object-contain" />
                        <span className="font-headline text-lg font-bold">
                            SG HEALTH PHARMA Portal{' '}
                            {isOfflineMode && <span className="text-xs font-normal text-muted-foreground">(Offline)</span>}
                        </span>
                    </Link>
                </div>
                {user && !isOfflineMode ? (
                    <DropdownMenu>
                        <DropdownMenuTrigger asChild>
                            <Button variant="ghost" className="flex h-auto items-center justify-start gap-3 px-3 py-2">
                                ...
                                <Avatar>
                                    <AvatarImage src={repAvatar?.imageUrl} data-ai-hint={repAvatar?.imageHint} />
